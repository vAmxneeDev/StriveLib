package it.vamxneedev.strivelib.gui;

import it.vamxneedev.strivelib.gui.elements.InventoryElements;
import it.vamxneedev.strivelib.gui.elements.InventoryProvider;
import it.vamxneedev.strivelib.gui.listeners.InventoryListener;
import it.vamxneedev.strivelib.gui.managers.InventoryManager;
...
public class StriveInventory {
    ...
    private StriveInventory parent;

    private List<InventoryListener<? extends Event>> listeners;
    private InventoryManager manager;

    private StriveInventory(InventoryManager manager) {
        this.manager = manager;
    }

    public Inventory open(Player player) { return open(player, 0); }
    public Inventory open(Player player, int page) {
        Optional<StriveInventory> oldInv = this.manager.getInventory(player);
        ...
        InventoryElements contents = new InventoryElements.Impl(this, player.getUniqueId());
    ...
    public Optional<StriveInventory> getParent() { return Optional.ofNullable(parent); }
    ...
    public static final class Builder {
        ...
        public Builder parent(StriveInventory parent) {
            this.parent = parent;
            return this;
        }
        ...
        public StriveInventory build() {
            if(this.provider == null)
                throw new IllegalStateException("The provider of the StriveInventory.Builder must be set.");

            InventoryManager manager = this.manager;

            if(manager == null)
                throw new IllegalStateException("The manager of the StriveInventory.Builder must be set, "
                        + "or the StriveLib should be loaded as a plugin.");

            StriveInventory inv = new StriveInventory(manager);
            inv.id = this.id;
